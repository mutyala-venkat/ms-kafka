...
class OrderController {
	...
	public Iterable<com.vmutyala.microservice.order.item.Item> items() {
	...
	//@Value("classpath:order.json") Resource resource;
	@RequestMapping(value = "/BookOrder", method = RequestMethod.GET)
	public ModelAndView OrderBook() throws JsonParseException, JsonMappingException, IOException {
		  //create ObjectMapper instance
        ObjectMapper objectMapper = new ObjectMapper();

        Order order;       

        InputStream is = TypeReference.class.getResourceAsStream("/json/order1.json");

			order = objectMapper.readValue(is, Order.class );
	    //    System.out.println(order);
			order.setOrderStatus("New");
			orderService.fireOrderCreatedEvent(order);
	...
	@RequestMapping(value = "/ComputerOrder", method = RequestMethod.GET)
	public ModelAndView OrderComputer() throws JsonParseException, JsonMappingException, IOException {
		  //create ObjectMapper instance
        ObjectMapper objectMapper = new ObjectMapper();
